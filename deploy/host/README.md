# Ouroboros Headless Host Operations (no Docker)
...
```bash
curl -fsSL https://raw.githubusercontent.com/keXXpert/ouroboros-headless/main/deploy/host/install.sh | sudo bash
```
...
```bash
REF="<tag-or-commit>"
curl -fsSL https://raw.githubusercontent.com/keXXpert/ouroboros-headless/main/deploy/host/install.sh | sudo bash -s -- --ref "${REF}"
```

Optional runtime user override:

```bash
curl -fsSL https://raw.githubusercontent.com/keXXpert/ouroboros-headless/main/deploy/host/install.sh | sudo bash -s -- --app-user myouro
```
...
sudo runuser -u ouroboros -- /home/ouroboros/repo/deploy/host/restore.sh /home/ouroboros/backups/ouroboros-headless-<timestamp>.tar.gz
```
