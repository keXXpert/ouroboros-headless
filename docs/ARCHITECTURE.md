# Ouroboros v5.2.3 — Architecture & Reference
...
- `widgets.js` — Widgets page for reviewed extension UI surfaces declared through `register_ui_tab`
- `about.js` — about page
...
3. The extension's ``register(api)`` makes one or more of
  ...
  - ``register_ui_tab(tab_id, …)`` exposes a reviewed widget declaration
    to the top-level Widgets page. v5.2.2 ships the narrow
    ``inline_card`` host used by the bundled weather widget; broader
    generic widget schemas are future work. Same-origin dynamic JS
    modules are intentionally not supported because they could call
...
| ``web/modules/marketplace.js`` | Marketplace UI lazily loaded by the Skills page when the user clicks the ``Marketplace`` tab. Provides lexical search, official-only filtering, cards, detail-modal (translated manifest table + adapter warnings/blockers + original ``SKILL.openclaw.md`` preview), and install/update/uninstall flows. |
